...
glActiveTexture(GL_TEXTURE0);
  ...
  glUniform4fv(loc_material_ambient,1,glm::value_ptr(ambient));
  glUniform4fv(loc_material_diffuse ,1,glm::value_ptr(diffuse));
  glUniform4fv(loc_material_specular,1,glm::value_ptr(specular));
  glUniform1f(loc_material_shininess,shininess);
  ...
  glEnableVertexAttribArray(loc_a_texcoord);
    ...
    glEnableVertexAttribArray(loc_a_normal);
     ...
     glDrawArrays(GL_TRIANGLES, 0, group.m_texcoords.size());
...
bool Object::load_simple_obj(const std::string& filename, std::string imgname)
{
	std::ifstream file(filename.c_str());
const char * imgnamed=imgname.data();
textureid = SOIL_load_OGL_texture
  ...
  programE = Shader::create_program("./shader/simple.vert", "./shader/simple.frag");
  loc_material_ambient  = glGetUniformLocation(programE, "material_ambient");
  loc_material_diffuse  = glGetUniformLocation(programE, "material_diffuse");
  loc_material_specular = glGetUniformLocation(programE, "material_specular");
  loc_material_shininess  = glGetUniformLocation(programE, "material_shininess");

  loc_u_texid = glGetUniformLocation(programE, "u_texid");
loc_a_texcoord = glGetAttribLocation(programE, "a_texcoord");
  if (textureid == 0)
  {
